...
public class SasinsuManager : ContentsManagerBase
{
    [Header("BossInfo")]
    ...
    {
        var serverData = ServerData.sasinsuServerTable.TableDatas[sasinsuTable.Stringid];

        if (string.IsNullOrEmpty(serverData.ConvertToString()) == false)
        {
            if (damageAmount.Value < double.Parse(serverData.ConvertToString()))
            {
                return;
            }
            else
            {
                serverData.score.Value = damageAmount.Value;

                ServerData.sasinsuServerTable.UpdateData(sasinsuTable.Stringid);
            }
        }
        else
        {
    ...
    private void RewardItem()
    {
        ...
        uiBossResultPopup.Initialize(damageAmount.Value, damageAmount.Value / 4000000000000000000);
    }
