...
using UnityEngine;
using UniRx;
public class UiYomulAttackUpgradeBoard : MonoBehaviour
{
    [SerializeField]
    ...
    {
        this.yomulAbilData = TableManager.Instance.YomulAbilTable.dataArray[2];
    ...
    private void Subscribe()
    {
        yomulServerData.level.AsObservable().Subscribe(e =>
        {
            ...
            abilDescription.SetText($"{CommonString.GetStatusName((StatusType)yomulAbilData.Abiltype)} {PlayerStats.GetYomulUpgradeValue(StatusType.AttackAdd)}");
    ...
    public void OnClickUpgradeButton()
    {
        ...
        if (ServerData.goodsTable.GetTableData(GoodsTable.GrowthStone).Value < price)
        {
        ...
        ServerData.goodsTable.GetTableData(GoodsTable.GrowthStone).Value -= price;
    ...
    public void OnClickUnlockButton()
    {
        ...
        if (ServerData.goodsTable.GetTableData(GoodsTable.GrowthStone).Value < yomulAbilData.Unlockprice)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.GrowthStone)}이 부족합니다.");
            return;
        }
        ...
        ServerData.goodsTable.GetTableData(GoodsTable.GrowthStone).Value -= yomulAbilData.Unlockprice;
