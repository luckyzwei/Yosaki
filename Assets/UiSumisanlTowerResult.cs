...
public class UiSumisanlTowerResult : MonoBehaviour
{
    [SerializeField]
    ...
    private string GetTitleText(ContentsState contentsState) 
    {
        switch (contentsState)
        {
            ...
            case ContentsState.Clear:
                if ((int)ServerData.userInfoTable.GetTableData(UserInfoTable.currentFloorIdx3).Value < (TableManager.Instance.sumisanTowerTable.dataArray.Length))
                {
