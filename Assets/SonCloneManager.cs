...
public class SonCloneManager : SingletonMono<SonCloneManager>
{
    [SerializeField]
    ...
    {
        int lastStage = (int)ServerData.userInfoTable.TableDatas[UserInfoTable.sonCloneClear].Value - (enemyMaxCount * 2);
    ...
    private IEnumerator StartTimer()
    {
        ...
        PopupManager.Instance.ShowAlarmMessage("분신들이 나타납니다.");
    ...
    public EnemyTableData GetEnemyTableData()
    {
        ...
        enemyData.Hp *= 10000000000000000000f;
    ...
    private void SpawnEnemies()
    {
        ...
        {
            var enemyObject = BattleObjectManager.Instance.GetItem($"GumGiSoul/SonClone") as Enemy;
    ...
    {
        remainEnemyText.SetText($"남은 분신:{spawnedEnemyList.Count}/{enemyMaxCount}");
    ...
    private void EndGame()
    {
        ...
        int pref = (int)ServerData.userInfoTable.TableDatas[UserInfoTable.sonCloneClear].Value;

        int updateValue = enemyDeadCount.Value;

        if (updateValue > pref)
        {
            ServerData.userInfoTable.UpData(UserInfoTable.sonCloneClear, updateValue, false);
        }
