...
public class UiMileageRewardCell : MonoBehaviour
{
    [SerializeField]
    ...
    private MileageRewardData tableData;
    ...
    private void Initialize()
    {
        ...
        tableData = TableManager.Instance.mileageReward.dataArray[tableId];
    ...
    public void OnClickExchangeButton()
    {
        ...
        int currentEventItemNum = (int)ServerData.goodsTable.GetTableData(GoodsTable.Mileage).Value;

        if (currentEventItemNum < tableData.Price)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.Mileage)}이 부족합니다.");
            return;
        }
        ...
        //로컬
        ServerData.goodsTable.GetTableData(GoodsTable.Mileage).Value -= tableData.Price;
