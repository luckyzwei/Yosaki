...
public class UiChildPassSystem : MonoBehaviour
{
    [SerializeField]
    private UiChildPassCell uiPassCellPrefab;

    [SerializeField]
    private Transform cellParent;

    private List<UiChildPassCell> uiPassCellContainer = new List<UiChildPassCell>();
    ...
    private void Update()
    {
        ...
        {
            ServerData.userInfoTable.GetTableData(UserInfoTable.killCountTotalChild).Value += 1000000;
        }
    }
#endif
    ...
    {
        var tableData = TableManager.Instance.childPass.dataArray;

        int interval = tableData.Length - uiPassCellContainer.Count;

        for (int i = 0; i < interval; i++)
        {
            var prefab = Instantiate<UiChildPassCell>(uiPassCellPrefab, cellParent);
    ...
    {
        int killCountTotal = (int)ServerData.userInfoTable.GetTableData(UserInfoTable.killCountTotalChild).Value;
