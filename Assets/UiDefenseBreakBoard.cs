...
public class UiDefenseBreakBoard : MonoBehaviour
{
    [SerializeField]
    ...
    {
        this.yomulAbilData = TableManager.Instance.YomulAbilTable.dataArray[0];
    ...
    private void Subscribe()
    {
        yomulServerData.level.AsObservable().Subscribe(e =>
        {
            ...
            abilDescription.SetText($"{CommonString.GetStatusName((StatusType)yomulAbilData.Abiltype)} {(PlayerStats.GetYomulUpgradeValue((StatusType)yomulAbilData.Abiltype, onlyType1: true) * 100f).ToString("F3")}%\n(초과된 방어력 수치만큼 추가 데미지)");
    ...
    public void OnClickUnlockButton()
    {
        ...
        if (ServerData.goodsTable.GetTableData(GoodsTable.Jade).Value < yomulAbilData.Unlockprice)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.Jade)}이 부족합니다.");
            return;
        }
        ...
        ServerData.goodsTable.GetTableData(GoodsTable.Jade).Value -= yomulAbilData.Unlockprice;
