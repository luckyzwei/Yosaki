...
public class UiLevelPassCell : MonoBehaviour
{
    ...
    public void OnClickAdRewardButton()
    {
        ...
        {
            PopupManager.Instance.ShowAlarmMessage("여우패스가 필요합니다!");
            return;
        }
    ...
    {
        bool hasIapProduct = ServerData.iapServerTable.TableDatas["levelpass"].buyCount.Value > 0;
    ...
    private void GetPassReward()
    {
        ...
        {
            SoundManager.Instance.PlaySound("Reward");
            LogManager.Instance.SendLog("여우패스", $"보상 {passInfo.id}");
            PopupManager.Instance.ShowAlarmMessage("보상을 수령했습니다!");
        });
    }
