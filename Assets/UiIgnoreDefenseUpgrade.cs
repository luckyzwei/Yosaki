...
using UnityEngine;
using UniRx;
public class UiIgnoreDefenseUpgrade : MonoBehaviour
{
    [SerializeField]
    ...
    {
        this.yomulAbilData = TableManager.Instance.YomulAbilTable.dataArray[3];
    ...
    public void OnClickUnlockButton()
    {
        ...
        if (ServerData.goodsTable.GetTableData(GoodsTable.TigerStone).Value < yomulAbilData.Unlockprice)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.TigerBossStone)}이 부족합니다.");
            return;
        }
