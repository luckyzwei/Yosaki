...
public class SumiFireManager : SingletonMono<SumiFireManager>
{
    [SerializeField]
    ...
    {
        int lastStage = (int)ServerData.userInfoTable.TableDatas[UserInfoTable.sumiFireClear].Value - (enemyMaxCount * 2);
    ...
    private IEnumerator StartTimer()
    {
        ...
        PopupManager.Instance.ShowAlarmMessage("오염된 꽃들이 나타납니다.");
    ...
    public EnemyTableData GetEnemyTableData()
    {
        ...
        enemyData.Hp *= 10000000000000000f;
    ...
    private void SpawnEnemies()
    {
        ...
        {
            var enemyObject = BattleObjectManager.Instance.GetItem($"GumGiSoul/SumiFire") as Enemy;
    ...
    {
        remainEnemyText.SetText($"남은 수미꽃:{spawnedEnemyList.Count}/{enemyMaxCount}");
    ...
    private void EndGame()
    {
        ...
        int pref = (int)ServerData.userInfoTable.TableDatas[UserInfoTable.sumiFireClear].Value;

        int updateValue = enemyDeadCount.Value;

        if (updateValue > pref)
        {
            ServerData.userInfoTable.UpData(UserInfoTable.sumiFireClear, updateValue, false);
        }
