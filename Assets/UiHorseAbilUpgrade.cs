...
using UnityEngine;
using UniRx;
public class UiHorseAbilUpgrade : MonoBehaviour
{
    [SerializeField]
    ...
    {
        this.yomulAbilData = TableManager.Instance.YomulAbilTable.dataArray[7];
    ...
    private void Subscribe()
    {
        yomulServerData.level.AsObservable().Subscribe(e =>
        {
            ...
            abilDescription.SetText($"{CommonString.GetStatusName((StatusType)yomulAbilData.Abiltype)} {PlayerStats.GetYomulUpgradeValue(StatusType.SuperCritical1DamPer, targetId: yomulAbilData.Id) * 100f}");
    ...
    public void OnClickUnlockButton()
    {
        ...
        if (ServerData.goodsTable.GetTableData(GoodsTable.HorseStone).Value < yomulAbilData.Unlockprice)
        {
            PopupManager.Instance.ShowAlarmMessage($"{CommonString.GetItemName(Item_Type.HorseStone)}이 부족합니다.");
            return;
        }
