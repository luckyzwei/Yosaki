...
public class UiLeeMuBoard : MonoBehaviour
{
    [SerializeField]
    ...
    private void Start()
    {
        bossTableData = TableManager.Instance.TwelveBossTable.dataArray[19];
    ...
    {
        var costumeServerData = ServerData.costumeServerTable.TableDatas["costume27"];
    ...
    public void OnClickGetButton()
    {
        ...
        param.Add("costume27", costumeServerData.ConvertToString());

        SendQueue.Enqueue(Backend.GameData.Update, CostumeServerTable.tableName, CostumeServerTable.Indate, param, e =>
        {
            if (e.IsSuccess())
            {
                PopupManager.Instance.ShowConfirmPopup(CommonString.Notice, "흑룡 외형 획득!", null);
            }
        });

        ServerData.costumeServerTable.SyncCostumeData("costume27");
